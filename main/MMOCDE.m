...
%% 初始化参数
n_var=size(VRmin,2);    %获取决策变量维度
Max_FES=Max_Gen*NP;     %计算最大函数评估次数
...
pos=VRmin+(VRmax-VRmin).*rand(NP,n_var); %在变量范围内初始化种群位置
%% 评估种群的适应度
fitness=zeros(NP,n_obj);
for i=1:NP
    fitness(i,:)=feval(func_name,pos(i,:));
end
fitcount=NP;            % 记录函数评估次数
pop=[pos,fitness];      % 位置和适应度合并为种群矩阵
%% 初始化外部存档
pop1=non_domination_scd_sort(pop,n_obj,n_var); %对种群非支配排序
EXA= pop1(pop1(:,n_var+n_obj+1)==1,:); % 提取第一层非支配解作为外部存档
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%循环
for I=1:Max_Gen
    fitpop= non_domination_scd_sort(pop,n_obj,n_var); % 对种群非支配排序
    for a=1:NP
        for b=1:NP
            if pos(a,1:n_var)== fitpop(b,1:n_var) % 获取每个个体的非支配等级
    ...
    for i=1:NP
        ...
        sortmiddlepop=non_domination_scd_sort(middlepop,n_obj,n_var); % TODO
