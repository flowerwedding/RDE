...
for iter = 1:iterations
    ...
    for i = 1:popsize
         ...
         % 策略2：选取前p=0.11的最优个体
         % p = 0.4 - 0.2*(iter/iterations)^1;% 精英比例p线性递减
         pNP = max(round(0.11 * P_A_PS), 2);
         ...
         elseif op_2(i) == 1
             ...
             % 当前个体 + F×(p-best-当前) + F×(父代1-父代2)
             vi(i, :) = X(i, :) + sf(i) .* (pbX(i, :) - X(i, :)) + sf(i) .* (X(parent1, :) - P_A(parent2, :));
             if LearnuseFlag && ~isempty(netF) % 如果启用了神经网络且网络已训练
                 if rand<pl % 以pl概率使用神经网络预测的F值
                     features = [X(i, :), X(parent1, :), P_A(parent2, :)];
                     F_pred = netF(features');
